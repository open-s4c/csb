...
const static char* UNIQUE_VAR(filenames)[1] = {"./tmp/rocksdb_db/OPTIONS-000035.dbtmp"};
...
//    fd: fd_dir (resource)
//    file: ptr[in, buffer] {
//      buffer: {2e 2f 74 6d 70 2f 72 6f 63 6b 73 64 62 5f 64 62 2f 4f 50 54 49 4f 4e 53 2d 30 30 30 30 33 35 2e 64 62 74 6d 70} (length 0x25)
//    }
//    flags: open_flags = 0x80241 (4 bytes)
//    mode: open_mode = 0x1ff (2 bytes)
//  ]
//  returns fd
memcpy((void*)(0x20d336c0ul+PTR_OFFSET), "./tmp/rocksdb_db/OPTIONS-000035.dbtmp", 37);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x20d336c0ul+PTR_OFFSET, /*flags=O_TRUNC|O_CREAT|O_CLOEXEC|O_WRONLY*/0x80241, /*mode=S_IXOTH|S_IWOTH|S_IROTH|S_IXGRP|S_IWGRP|S_IRGRP|S_IXUSR|S_IWUSR|0x100*/0x1ff);
...
//    }
//  ]
	res = syscall(__NR_fstatfs, /*fd=*/UNIQUE_VAR(ctx->r)[0], /*buf=*/0x20d33700ul+PTR_OFFSET);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
...
//  ]
	res = syscall(__NR_write, /*fd=*/UNIQUE_VAR(ctx->r)[0], UNIQUE_VAR(ctx->writeBuffer), /*count=*/0x1e58ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fsync arguments: [
//    fd: fd (resource)
//  ]
...
//  ]
//  returns fd
memcpy((void*)(0x20d35600ul+PTR_OFFSET), "./tmp/rocksdb_db/OPTIONS-000035.dbtmp", 37);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x20d35600ul+PTR_OFFSET, /*flags=O_CREAT|O_CLOEXEC*/0x80040, /*mode=S_IXOTH|S_IWOTH|S_IROTH|S_IXGRP|S_IWGRP|S_IRGRP|S_IXUSR|S_IWUSR|0x100*/0x1ff);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
	if (res != -1)
...
//  ]
	res = syscall(__NR_read, /*fd=*/UNIQUE_VAR(ctx->r)[1], /*buf=*/0x20d35700ul+PTR_OFFSET, /*count=*/0x80000ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  read arguments: [
//    fd: fd (resource)
//    buf: ptr[out, buffer] {
...
//  ]
	res = syscall(__NR_read, /*fd=*/UNIQUE_VAR(ctx->r)[1], /*buf=*/0x20d37580ul+PTR_OFFSET, /*count=*/0x7e000ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  close arguments: [
//    fd: fd (resource)
//  ]
...
//  ]
memcpy((void*)(0x20d37a40ul+PTR_OFFSET), "./tmp/rocksdb_db/OPTIONS-000035.dbtmp", 37);
	res = syscall(__NR_newfstatat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x20d37a40ul+PTR_OFFSET, /*statbuf=*/0x20d37a80ul+PTR_OFFSET, /*flag=*/0ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  renameat arguments: [
...
//    }
//  ]
memcpy((void*)(0x20d37b00ul+PTR_OFFSET), "/tmp/rocksdb_db/OPTIONS-000035.dbtmp\000", 37);
memcpy((void*)(0x20d37b40ul+PTR_OFFSET), "/tmp/rocksdb_db/OPTIONS-000036\000", 31);
	res = syscall(__NR_renameat, /*oldfd=*/0xffffff9c, /*old=*/0x20d37b00ul+PTR_OFFSET, /*newfd=*/0xffffff9c, /*new=*/0x20d37b40ul+PTR_OFFSET);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
	return 0;
}
