...
static inline void parse_net_addr(const char *addr_env, const char *port_env,
                                  struct sockaddr_in *dst, bool *initialized,
                                  uint16_t sin_port) {
  ...
  if (conn_addr_str) {
    ...
    int r = inet_pton(dst->sin_family, conn_addr_str, &dst->sin_addr.s_addr);
    if (r == 1) {
      *initialized = true;
    } else if (r == -1) {
      ...
      fprintf(stderr, "inet_pton(connect addr): Not in presentation format.");
      exit(-1);
    }
  }
