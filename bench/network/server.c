...
static void unregister(struct conn_data *d, int efd) {
...
static void config_wait(struct conn_data *d, int efd) {
  size_t step = d->step;
...
static void advance_step(struct conn_data *d) {
  d->n++;
...
static void readwrite(struct epoll_event *ev, int efd) {
...
void prepare_buffer(struct extracted_op *ops, long nops) {
  long max_sz = 0;
...
int main(int argc, char *argv[]) {
  ...
  if (prog_file) {
    ...
    if (!program) {
      fprintf(stderr, "Failed to load the operation sequence from file %s\n",
              prog_file);
    }
  }
